...
useEffect(() => {
  ...
  return (
    <div>
    <div className={`md:w-200 w-full md:rounded-xl col-start-1 row-start-1 overflow-hidden shadow-sm border border-gray-100" id="Display ${
          ...
          <p className="text-sm  mt-1">Customize your visual experience</p>
        </div>
        ...
        {displayOptions.map((option, index) => (
          ...
          theme==="dark" ? "text-white" : "text-gray-900"
              ...
              <Icon 
                icon={openSetting === option.id ? "mdi:chevron-up" : "mdi:chevron-down"} 
                className={`w-5 h-5 text-gray-400 transition-transform duration-200 ${
                  openSetting === option.id ? 'rotate-180' : ''
                }`}
              />
            </div>
          ...
          theme==="dark" ? "bg-gray-900" : "bg-white"
                    ...
                    <>
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          >
                            <Icon icon="mdi:white-balance-sunny" className="w-5 h-5 mr-2" />
                            <span className="font-medium">Light</span>
                          </button>
                          
                          <button
                            onClick={() => handleThemeChange('dark')} //Handle Theme Change Toggle
                            className={`flex items-center justify-center px-4 py-3 rounded-xl border-2 transition-all duration-200 ${
                              theme === 'dark'
                                ? 'bg-blue-50 border-blue-500 text-blue-700 shadow-sm'
                                : 'bg-gray-50 border-gray-200 text-gray-700 hover:bg-gray-100 hover:border-gray-300'
                          ...
                          >
                            <Icon icon="mdi:moon-waning-crescent" className="w-5 h-5 mr-2" />
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          {(['small', 'medium', 'large'] as const).map((size) => (
                            <button
                              key={size}
                              onClick={() => handleFontChange(size)} //Changes the fontsize
            ...
            {index !== displayOptions.length - 1 && <hr className="border-gray-100" />}
          </div>
        ))}
      </div>
    </div>
