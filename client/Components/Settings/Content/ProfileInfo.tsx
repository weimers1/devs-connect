...
function ProfileInfo() {
  ...
  const handleEditProfileClick = () => { //Navigating to the profile page so a user can edit their profile
      navigate('/profile?showProfModal=true');
  }
 
//Profile State for Update When A User Updates 
  const [profileSettings, setProfileSettings] = useState({ //What user is typing/editing right now
    //DB Equivalent 
    firstName: '', 
    lastName: '',
    location: '',
  ...
  const [originalData, setOriginalData] = useState({ //What's actually saved in database (Used for cancel functionality) || prevents Data loss
  firstName: '', 
  lastName: '',
  location: '',
  age: '',
...
useEffect(() => {
  ...
  if (githubStatus === 'success' && githubId && githubEmailParam !== 'no-email') {
    setGithubConnected(true);
    setGithubEmail(githubEmailParam || '');
    setGithubUsername(githubUsernameParam || '');
    
    // Save GitHub data to database
    saveGitHubData(githubId, githubUsernameParam || '', githubEmailParam || '');
 ...
 // Save GitHub data to database
 const saveGitHubData = async (githubId: string, githubUsername: string, githubEmail: string) => {
   try {
     const response = await fetch('http://localhost:6969/user/link-github', {
       method: 'POST',
       credentials: 'include',
       headers: {
  ...
  return (
    <div>
      <div className={`md:w-200 w-full md:rounded-xl mt-4 col-start-1 row-start-1 overflow-hidden shadow-sm border ${
        ...
        <div className={`p-6 border-b border-gray-100 ${
          ...
          <p className="text-sm">Manage your personal details and verification</p>
        </div>
        ...
        {sections.map((section, index) => (
          ...
          theme==="dark" ? "text-white" : "text-gray-900"
              ...
              <Icon 
                icon={openSetting === section.id ? "mdi:chevron-up" : "mdi:chevron-down"} 
                className={`w-5 h-5 text-gray-400 transition-transform duration-200 ${
                  openSetting === section.id ? 'rotate-180' : ''
                }`}
              />
            </div>

            {openSetting === section.id && (
              <div className={`mx-4 mb-4  rounded-lg shadow-sm border${
                theme==="dark" ? "bg-gray-900" : "bg-white"
              }`}>
                <div className="p-6 space-y-4">
                  {section.id === 'name-location' && (
                   ...
                   <button className="border-gray-500" onClick={()=> handleCancel()}>Cancel</button>
                      </div>
                       <button 
  onClick={handleSave} 
...
>  {loading ? (<>
    ...
    'Save'
                  ...
                  {section.id === 'verifications' && (
                    <div>
                      <p className="text-sm text-gray-600 mb-4">Verify your identity to build trust in the community.</p>
                      <div className="space-y-3">
                        <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg bg-green-50">
                          <div className="flex items-center">
                            <Icon icon="mdi:email" className="w-5 h-5 text-green-600 mr-3" />
                            <div>
                              <span className="font-medium text-gray-900">Email Verified</span>
                              <p className="text-sm text-gray-600">ethanmclaughlin24@gmail.com</p>
                            </div>
                          </div>
                        ...
                        <div className={`flex items-center justify-between p-4 border rounded-lg ${
                          ...
                          <div className="flex items-center">
                            <Icon 
                              ...
                              className={`w-5 h-5 mr-3 ${
                                githubConnected ? 'text-green-600' : 'text-gray-400'
                              }`} 
                            />
                            <div>
                          ...
                          {githubConnected ? (
                            ...
                            <button 
                              onClick={() => window.location.href = 'http://localhost:6969/oauth/github'}
