...
function AccountManagement() {
  ...
  return (
      <div id="Management" className={`md:w-200 w-full md:rounded-xl col-start-1 row-start-1 overflow-hidden shadow-sm border border-gray-100"  ${
     ...
     <div className={`p-6 border-b border-gray-100 ${
        ...
        <p className="text-sm  mt-1">Manage your account settings and data</p>
      </div>
      ...
      {Management.map((item, index) => (
        <div key={item.id}>
          ...
          theme==="dark" ? "text-white" : "text-gray-900"
              ...
              <h3 className={`font-medium ${item.id === 'delete-account' ? 'text-red-600' : ''}`}>
                {item.name}
              </h3>
              </div>
                  ...
                  <div>
                    <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
                      <div className="flex items-start">
                        <Icon icon="mdi:alert-circle" className="w-5 h-5 text-red-600 mr-3 mt-0.5 flex-shrink-0" />
                        <div>
                          ...
                          <div className="text-sm text-red-700 space-y-1">
                            <div>• All your posts and comments will be removed</div>
                            <div>• Your community memberships will be cancelled</div>
                            <div>• Your hackathon history will be deleted</div>
                            <div>• Any active subscriptions will be cancelled</div>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <label className="block text-sm font-medium  mb-2">
                          Type <span className="font-mono  px-1 rounded">DELETE</span> to confirm
                        </label>
                        </div>
                        <input 
                          type="text" 
                ...
                {item.id === 'logout' && (
                  <div>
                    <div className="flex items-center p-4 bg-blue-50 border border-blue-200 rounded-lg mb-4">
                      <Icon icon="mdi:information" className="w-5 h-5 text-blue-600 mr-3" />
          ...
          {index !== Management.length - 1 && <hr className="border-gray-100" />}
        </div>
      ))}
    </div>
  )
