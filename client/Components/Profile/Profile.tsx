...
const handleImageUpload = async (file) => {
  ...
  try {
    ...
    const uploadResponse = await fetch('http://localhost:6969/api/upload/profile-image', {
      method: 'POST',
      body: formData
    });
    ...
    if (uploadResult.success) {
      ...
      const saveResponse = await fetch('http://localhost:6969/api/settings/profile-image', {
        method: 'PUT',
        headers: {
...
useEffect(() => {
  ...
  if(urlParams.get('showCertModal') === 'true') { //Checking if the redirect was for the certification Modal
    setShowCertModal(true);
  } else if(urlParams.get('showProfModal') === 'true') { //Checking if the redirect was for the profile Model
 ...
 const handleSave = async () => {
  ...
  try {
    ...
    const certPayload = {
      ...
      dateEarned: combineDateFields(certData.issuedMonth, certData.issuedYear),
      dateExpiration: combineDateFields(certData.expiryMonth, certData.expiryYear),
      ...
      {showCertModal && (
       ...
       >
          <div className="bg-white rounded-lg p-6 w-170 max-h-fit mr-5">
            <h2 className="text-xl font-bold mb-2">Add Certification</h2>
...
>
  {certSaving ? (
    ...
    'Save'
        ...
        {showProfModal && (
          <div className="fixed inset-0 flex items-start justify-center pt-8 z-[9999]"
